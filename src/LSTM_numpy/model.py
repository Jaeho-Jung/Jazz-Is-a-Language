"""
NumPy LSTM Model for Jazz Solo Generation

Integrates embeddings, LSTM, and output heads into a complete model.
"""
...
from src.LSTM_numpy.layers.embedding import Embedding
from src.LSTM_numpy.layers.linear import Linear
from src.LSTM_numpy.layers.lstm import LSTM
from src.LSTM_numpy import config


class JazzLSTM:
    """
    Complete LSTM model for jazz solo generation.
    ...
    def __init__(self, num_dur_classes):
        ...
        self.pitch_head = Linear(config.LSTM_HIDDEN_SIZE, config.VOCAB_SIZE_PITCH)
        self.dur_head = Linear(config.LSTM_HIDDEN_SIZE, num_dur_classes)
    ...
    def forward(self, features):
        ...
        self._cached_seq_len = lstm_input.shape[1]  # Cache seq_len for backward
        h_seq, h_final, c_final = self.lstm.forward(lstm_input)
    ...
    def backward(self, grad_pitch_logits, grad_dur_logits):
        ...
        grad_h_seq = np.zeros((batch_size, seq_len, config.LSTM_HIDDEN_SIZE))
        grad_h_seq[:, -1, :] = grad_last_hidden
        
        # Backprop through LSTM
        grad_lstm_input = self.lstm.backward(grad_h_seq)
        ...
        for key, size in zip(embed_keys, embed_sizes):
            grad_emb = grad_lstm_input[:, :, idx:idx+size]
    ...
    def get_all_params(self):
        ...
        # LSTM
        params['lstm_cell'] = self.lstm.cell.get_params()
    ...
    def get_all_grads(self):
        ...
        # LSTM
        grads['lstm_cell'] = self.lstm.cell.get_grads()
    ...
    def set_all_params(self, params):
        ...
        # LSTM
        if 'lstm_cell' in params:
            self.lstm.cell.set_params(params['lstm_cell'])
